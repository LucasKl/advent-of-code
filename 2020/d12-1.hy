...
(defn move [ship command]
  (if (= command.action "N") (Ship ship.x (+ ship.y command.value) ship.dirx ship.diry)
      (= command.action "S") (Ship ship.x (- ship.y command.value) ship.dirx ship.diry)
      (= command.action "E") (Ship (+ ship.x command.value) ship.y ship.dirx ship.diry)
      (= command.action "W") (Ship (- ship.x command.value) ship.y ship.dirx ship.diry)
...
(print "D12-1:" (.manhatten (reduce move commands (Ship 0 0 1 0))))
